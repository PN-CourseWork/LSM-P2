...
def extract_resources(combo: Dict[str, Any], resource_template: Dict[str, Any]) -> Tuple[frozenset, Dict[str, Any]]:
    """
...
def generate_arrays(config_path: Path, output_dir: Path = None):
    ...
    for group_name, group_config in config.items():
        ...
        if "sweep" not in group_config and "static_args" not in group_config and "sweep_paired" not in group_config:
             continue
        ...
        for i, (sig, data) in enumerate(groups.items()):
            ...
            bsub_args.append(f"-R \"span[ptile=24]\"") # Default, could be configurable
            
            # Output logs
            bsub_args.append(f"-o logs/lsf/%J_%I.out")
            bsub_args.append(f"-e logs/lsf/%J_%I.err")
