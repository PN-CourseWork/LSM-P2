...
class TuiApp:
    ...
    def draw_hpc_dashboard(self):
        ...
        for i, job in enumerate(visible_jobs):
            ...
            if y <= content_end_y:
                ...
                status_color = (
                    term.green
                    if job.status == "RUN"
                    else term.yellow if job.status == "PEND" else term.white
                )
        ...
        if self.hpc_pane_focus == 0:
            ...
            if "Generate" in label:
                content = [
                    "Generates LSF job script files based on",
                    "packs.yaml configuration.",
                    "",
                    "Interactive wizard will ask for groups.",
                ]
            else:
                content = [
                    "Submits generated .pack files to the",
                    "LSF scheduler using 'bsub'.",
                    "",
                    "You can review the file before submitting.",
                ]
        else:
    ...
    def draw_actions_dashboard(self):
        ...
        for y in range(3, term.height - 1):
            print(term.move_xy(mid_x, y) + term.cyan("│"))
        ...
        print(
            term.move_xy(mid_x + 2, 7 + len(items) + 2)
            + term.bright_black("Press Enter to select action")
        )
